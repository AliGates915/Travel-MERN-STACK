...
export const createPackageList = async (req, res, next) => {
  ...
  try {
    const newPackageList = new PackageList({
      packageName: req.body.packageName, // Ensure the property matches your schema
...
export const getPackageList = async (req, res, next) => {
  try {
    ...
    const packageList = await PackageList.findById(packageListId).populate('packageName tourType facilities');
    if (!packageList) {
      return res.status(404).json({ message: "Package list not found" });
    }
...
export const getAllPackageList = async (req, res, next) => {
  try {
    const packageLists = await PackageList.find().populate('packageName tourType facilities');
